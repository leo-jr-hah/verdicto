...
import { Radio, RefreshCw, Shield, Zap, Code, Copy, Check, Wifi, ChevronDown, ChevronUp, Database, Activity, AlertTriangle, Scale, Swords, Gavel } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { fetchOracleVerdicts, fetchDisputes, fileDispute, triggerRetrial, type OracleVerdict, type OracleStats, type Dispute } from '../services/api';
...
export const OracleView: React.FC = () => {
  ...
  const [disputeModal, setDisputeModal] = useState<string | null>(null); // assetId being disputed
  const [disputeReason, setDisputeReason] = useState('');
  const [disputeLoading, setDisputeLoading] = useState(false);
  const [retrialLoading, setRetrialLoading] = useState<string | null>(null); // disputeId being retried

  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const [data, disputesData] = await Promise.all([
        fetchOracleVerdicts(),
        fetchDisputes(),
      ]);
  ...
  return (
    <>
                    ...
                    {isExpanded && (
                      ...
                      >
                        ...
                        <div style={{ padding: '0 20px 16px', display: 'flex', alignItems: 'center', gap: '12px' }}>
                          <button
                            onClick={(e) => { e.stopPropagation(); setDisputeModal(v.assetId); }}
                            style={{
                              display: 'flex', alignItems: 'center', gap: '6px',
                              padding: '8px 16px', borderRadius: '8px',
      ...
      <motion.div
        ...
        style={{
          borderRadius: '12px', border: '1px solid var(--border-color)',
          background: 'var(--bg-primary)', overflow: 'hidden', marginTop: '32px',
        }}
      >
        <div style={{
          padding: '16px 20px', borderBottom: '1px solid var(--border-color)',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
            <Scale size={12} style={{ color: '#8B5CF6' }} />
            <span style={{ fontSize: '0.75rem', color: 'var(--text-tertiary)' }}>Adversarial Panel</span>
          </div>
        </div>
        ...
        {disputes.length === 0 ? (
          <div style={{ padding: '60px 20px', textAlign: 'center', color: 'var(--text-secondary)' }}>
            ...
            <p style={{ margin: '0 0 4px', fontWeight: 500 }}>No disputes filed yet</p>
            <p style={{ margin: 0, fontSize: '0.85rem', color: 'var(--text-tertiary)' }}>
              Challenge any verdict by staking CSPR. The adversarial panel re-runs the case independently.
            </p>
          </div>
        ) : (
          <div>
              ...
              <div style={{ padding: '20px 24px' }}>
                ...
                <div style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '4px' }}>
                    ...
                    <span style={{ fontSize: '0.8rem', fontWeight: 600, color: '#F59E0B' }}>Stake: 5 CSPR</span>
                  </div>
                  <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', lineHeight: 1.4 }}>
                    If the verdict is overturned, you get your stake back. If upheld, your stake is distributed to the original jurors.
                  </div>
                </div>
              </div>
              ...
              <div style={{
                ...
                <button
                  ...
                  style={{
                    ...
                    border: 'none', background: disputeReason.trim() && !disputeLoading ? 'linear-gradient(135deg, #EF4444, #F97316)' : 'var(--bg-secondary)',
                    color: disputeReason.trim() && !disputeLoading ? '#fff' : 'var(--text-tertiary)',
                    cursor: disputeReason.trim() && !disputeLoading ? 'pointer' : 'not-allowed',
                    fontSize: '0.85rem', fontWeight: 600,
                  }}
                >
