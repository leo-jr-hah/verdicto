...
import { Radio, RefreshCw, Shield, Zap, Code, Copy, Check, Wifi, ChevronDown, ChevronUp, Database, Activity } from 'lucide-react';
...
export const OracleView: React.FC = () => {
  ...
  const [stats, setStats] = useState<OracleStats>({ totalVerdicts: 0, freshVerdicts: 0, avgConfidence: 0, totalQueries: 0 });
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setStats(data.stats || { totalVerdicts: 0, freshVerdicts: 0, avgConfidence: 0, totalQueries: 0 });
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: '16px' }}>
          ...
          <div>
            ...
            <p style={{ margin: 0, fontSize: '0.9rem', color: 'var(--text-secondary)', lineHeight: 1.6 }}>
              ...
              Any Casper dApp (lending protocols, DAO treasuries, insurance contracts) can query it
              via cross-contract call and pay a 0.1 CSPR micropayment. No API key. No human in the loop.
              Agent-to-agent.
            </p>
          </div>
        </div>
                    ...
                    {isExpanded && (
                      ...
                      >
                        <div style={{ padding: '16px 20px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px' }}>
                          ...
                          <div>
                            <div style={{ fontSize: '0.75rem', color: 'var(--text-tertiary)', marginBottom: '4px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Agent Weights</div>
                            <div style={{ fontSize: '0.85rem', color: 'var(--text-primary)', fontFamily: 'monospace' }}>{v.agentWeights || 'N/A'}</div>
                          </div>
                          <div>
